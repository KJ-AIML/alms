"""Probe entrypoint for openai-compatible-custom."""
...
from .client import build_live_client, build_offline_client, client_retry_config
from .execute import execute
from .serialize import write_capture
from .translate import to_chat_completions_request
...
OFFLINE_BASE_URL = "https://compat.example.test/v1"
...
def _fail(msg: str, code: int) -> int:
    sys.stderr.write(f"[probe:openai-compatible-custom] {msg}\n")
    return code
...
def run(request: dict) -> tuple[int, dict | None]:
    ...
    endpoint_id = lane.get("endpoint_id") or "compat-openai-offline"
    ...
    if mock_mode:
        ...
        api_key = os.environ.get("ALMS_COMPAT_OPENAI_API_KEY")
        base_url = os.environ.get("ALMS_COMPAT_OPENAI_BASE_URL")
    ...
    kwargs, meta = to_chat_completions_request(fixture, model, controls.get("max_output_tokens"))
    # Force zero SDK retries already configured; measure attempts via wire when offline.
    capture = execute(client, kwargs, attempt_count=0)
    if wire is not None:
    ...
    retry_info = client_retry_config(client)
    meta["retry_config"] = retry_info
