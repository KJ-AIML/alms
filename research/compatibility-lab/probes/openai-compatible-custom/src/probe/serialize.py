"""Serialize endpoint-boundary evidence without secrets or full base URLs."""
...
def write_capture(
    *,
    ...
    request_path = raw / "openai_compatible_request.json"
    ...
    if isinstance(response_body, dict):
        ...
        if returned_model is None and capture.kind == "stream" and capture.events:
    ...
    boundary = {
        ...
        "usage_fields": (response_body or {}).get("usage")
        if isinstance(response_body, dict)
        else None,
        "finish_or_terminal_state": _finish(response_body, capture),
        "tool_calls": _tools(response_body),
...
def _finish(response_body: Any, capture: RawCapture) -> Any:
    if capture.kind == "stream" and capture.events:
        last = capture.events[-1].get("data") or {}
...
def _tools(response_body: Any) -> list:
