...
Reads one JSON request, runs one fixture x lane operation through LangChain, writes raw
...
The live path is present but fail-closed and is NOT exercised in the P0.5 offline session.
Only a direct OPENAI_API_KEY satisfies the live credential gate; OPENROUTER_API_KEY does not,
and no base_url is ever configured.
"""
...
from .client import OfflineChatModel, build_chat_model
...
def _fail(msg: str, code: int) -> int:
    sys.stderr.write(f"[probe:langchain] {msg}\n")
    return code
...
def run(request: dict) -> tuple[int, dict | None]:
    ...
    if mock_mode:
        ...
        chat_model = OfflineChatModel(scenario=_scenario_for(fixture))
    else:
        ...
        # Direct OpenAI credential ONLY. OPENROUTER_API_KEY or any gateway key must NOT
        # satisfy this gate, and no base_url is ever set.
        ...
        chat_model = build_chat_model(api_key, controls.get("timeout_ms", 30000), model)

    operation, _meta = to_operation(fixture, model, controls.get("max_output_tokens"))
    capture = execute(chat_model, operation)
