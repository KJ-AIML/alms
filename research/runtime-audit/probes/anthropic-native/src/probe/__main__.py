...
Reads one JSON request, runs one fixture x lane operation through the Anthropic Messages API,
...
The live path is present but fail-closed and is NOT exercised in the P0.6A offline session.
Only a direct ANTHROPIC_API_KEY satisfies the live credential gate; an OpenRouter key does
not, and no base_url is ever configured (no OpenRouter/Bedrock/Vertex/gateway redirect).
"""
...
from .client import FakeAnthropicClient, build_client
...
def _fail(msg: str, code: int) -> int:
    sys.stderr.write(f"[probe:anthropic-native] {msg}\n")
    return code
...
def run(request: dict) -> tuple[int, dict | None]:
    ...
    if mock_mode:
        model = model or "claude-mock-model"
        client = FakeAnthropicClient(scenario=_scenario_for(fixture))
    else:
        ...
        # Direct Anthropic credential ONLY. An OpenRouter or gateway key must NOT satisfy this
        # gate, and no base_url is ever set.
        api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
            _write_blocked(out_dir, "BLOCKED_CREDENTIAL", "ANTHROPIC_API_KEY not present")
    ...
    manifest = write_capture(
        ...
        provider=lane.get("provider", "anthropic"),
        model=model,
        started_at=started_at,
        redaction_mode=request.get("redaction_mode", "strict"),
        execution_mode=execution_mode,
        secret_values={v for v in [os.environ.get("ANTHROPIC_API_KEY")] if v},
    )
