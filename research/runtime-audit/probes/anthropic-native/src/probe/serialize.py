"""Serialize raw Anthropic evidence to disk and build a schema-valid probe-response manifest.
...
is a small envelope the harness's Anthropic normalizer understands; it carries the native
Message / stream events / error plus retry introspection, never a normalized interpretation.
"""
...
def _package_versions() -> dict[str, str]:
    try:
        return {"anthropic": version("anthropic")}
...
def write_capture(
    *,
    ...
    request_path = raw / "anthropic_request.json"
    _write_json(request_path, red_request)

    envelope = _response_envelope(capture)
    red_response, rf = redact(envelope, secret_values)
    fields_redacted.update(rf)
    fname = {"stream": "anthropic_events.json", "error": "anthropic_error.json"}.get(
        capture.kind, "anthropic_response.json"
    )
    ...
    manifest = {
        ...
        "provider": provider,
        "model": model,  # REQUESTED model; the provider-returned model lives in the raw message
