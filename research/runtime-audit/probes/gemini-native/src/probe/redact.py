"""Narrow secret redaction for raw Gemini artifacts (DevSpec Section 85).

Local to the probe (probes are isolated and must not import the harness). Masks API keys,
Authorization/Bearer, x-goog-api-key, cookies, and signed-URL signatures; preserves benign
evidence such as interaction IDs, model IDs, usage, and synthetic content. Byte-identical
policy to the other probes so redaction is consistent across lanes.
"""
...
_PATTERNS = (
    ...
    re.compile(r"AIza[A-Za-z0-9_\-]{10,}"),  # Google API key shape
