"""LiteLLM SDK probe test fixtures, including an in-process network tripwire.

The probe package sets LITELLM_LOCAL_MODEL_COST_MAP on import so importing litellm never touches
the network; this conftest additionally fails any outbound socket during tests.
"""
...
# LiteLLM routes by provider prefix; an unqualified model raises. Offline tests use a
# provider-qualified synthetic model string (no hidden default).
_LANE = {
    "lane_id": "litellm-sdk",
