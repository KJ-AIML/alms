...
Includes the critical guarantee that LiteLLM's stdout promo/info banners do NOT corrupt the
manifest: real stdout must carry ONLY the one-line JSON manifest the harness parses.
"""
...
def test_mock_subprocess_stdout_is_clean_single_line_manifest(tmp_path):
    ...
    # Exactly one non-empty stdout line, and it parses as the manifest (no litellm promo leaked in).
    lines = [ln for ln in result.stdout.splitlines() if ln.strip()]
    assert len(lines) == 1, f"stdout not clean: {result.stdout!r}"
    manifest = json.loads(lines[0])
    assert manifest["probe_id"] == "litellm-sdk"
...
def test_gateway_key_does_not_satisfy_credential_gate(tmp_path):
    # An OpenRouter / gateway key must NOT unlock the live path: only a direct OPENAI_API_KEY does.
    # This lane must never be silently redirected to a proxy or gateway.
    fx = write_fixture(tmp_path / "fx.json")
    out = tmp_path / "out"
    req = write_request(tmp_path / "req.json", fx, out, lane=_LIVE_LANE, controls=_LIVE_CONTROLS)
    env = {k: v for k, v in os.environ.items() if k != "OPENAI_API_KEY"}
...
def test_litellm_importable_in_probe_env():
    assert importlib.util.find_spec("litellm") is not None


def test_fastapi_absent_confirms_sdk_not_proxy():
    # This lane audits the LiteLLM SDK, not the Proxy Server. fastapi (a proxy-server dependency)
    # is deliberately NOT installed; its absence is part of the SDK/proxy boundary.
