...
| D-006 | Custom `base_url` / OpenRouter compatibility lanes | Valid separate evidence class for gateway-routed providers. Must not substitute for G2 (`OPENAI_API_KEY` direct) or G3 (non-OpenAI native). Probes enforce direct-credential gates; OpenRouter keys do not unlock native live paths. Offline design only until authorized. | Compatibility / gateway audit DevSpec (distinct from native lanes) | Explicit authorization for gateway lanes; separate fixture tier and credential policy; G2/G3 remain satisfied by direct native evidence |
| D-007 | EmbeddingRuntime sibling DevSpec (Q-005) | Q-005 open: embedding scope not decided. `EMBED-001` is OPTIONAL tier, separate from six generation fixtures. Assumption A-007 open. Gate matrix EXIT-12 requires recommendation or explicit evidence-backed deferral; deferral doc pending P0.9-q005. | Sibling `EmbeddingRuntime` DevSpec (first alpha or later) per Q-005 outcome | P0.9-q005 deferral slice; live `EMBED-001` evidence if authorized; A-007 outcome update |
